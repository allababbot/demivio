 ...
 */
function calculateHumanScore(result: {
  ...
  // --- Weighted composite ---
  const composite = ppnScore * 0.6 + priceScore * 0.15 + qtyScore * 0.1 + discScore * 0.15;
...
function createResult(
  ...
  const metadata: ResultMetadata = {
    ppnDifferencePercent: targetPpn.isZero() ? new Decimal(0) : ppnDifference.div(targetPpn).mul(100),
 ...
 */
function calculateRequiredUnitPrice(targetPpn: Decimal, quantity: Decimal, discount: Decimal): Decimal {
 ...
 */
function calculateRequiredDiscount(targetPpn: Decimal, unitPrice: Decimal, quantity: Decimal): Decimal {
  const requiredDPP = targetPpn.div(PPN_EFFECTIVE_RATE);
 ...
 */
function calculateRequiredQuantity(targetPpn: Decimal, unitPrice: Decimal, discount: Decimal): Decimal {
 ...
 * Estimate number of combinations, accounting for locked parameters
 *
 * NOTE: This function uses Decimal comparisons to match the logic in
 * estimateCombinations. It does NOT use simple number comparison to
...
export function estimateCombinationsSimple(config: {
  ...
  const qtySteps = Math.max(
    1,
    Math.floor(config.quantityMax.sub(config.quantityMin).div(config.quantityStep).toNumber()) + 1,
  );
  const discSteps = Math.max(
    1,
    Math.floor(config.discountMax.sub(config.discountMin).div(config.discountStep).toNumber()) + 1,
  );

  if (priceLocked && discountLocked) return qtySteps;
  if (priceLocked) return qtySteps;
  if (discountLocked) return qtySteps;
  return discSteps * qtySteps;
}
...
export function runSimulation(
  ...
  const processPriceDiscount = (unitPrice: Decimal, discount: Decimal): void => {
    ...
    const qtysToTry = [roundedQty, roundedQty.add(config.quantityStep), roundedQty.sub(config.quantityStep)];
  ...
  const shouldStop = (): boolean =>
    perfectCount >= config.topNResults || results.length >= enoughResults || (shouldCancel?.() ?? false);

  if (priceLocked && discountLocked) {
    // Only Qty is variable
    processPriceDiscount(priceMin, discountMin);
  } else if (priceLocked && qtyLocked) {
    // Only Discount is variable
    processPriceDiscount(priceMin, discountLocked ? discountMin : discountMax);
    // Actually, processQtyUnitPrice(qtyMin, priceMin) also works
    processQtyUnitPrice(config.quantityMin, priceMin);
  } else if (discountLocked && qtyLocked) {
    // Only Price is variable
    processQtyDiscount(config.quantityMin, discountMin);
  } else if (priceLocked) {
    // Qty and Discount can vary. Solve for one, iterate the other based on step count.
    const qtySteps = config.quantityMax.sub(config.quantityMin).div(config.quantityStep).floor().toNumber() + 1;
    const discountSteps = discountMax.sub(discountMin).div(config.discountStep).floor().toNumber() + 1;

    if (qtySteps <= discountSteps) {
      for (let q = config.quantityMin; q.lte(config.quantityMax); q = q.add(config.quantityStep)) {
        if (shouldStop()) break;
        processQtyUnitPrice(q, priceMin);
        count++;
        if (onProgress && count % reportInterval === 0) onProgress(Math.min(count / totalEstimate, 0.99));
      }
    } else {
      for (let d = discountMin; d.lte(discountMax); d = d.add(config.discountStep)) {
        if (shouldStop()) break;
        processPriceDiscount(priceMin, d);
        count++;
        if (onProgress && count % reportInterval === 0) onProgress(Math.min(count / totalEstimate, 0.99));
      }
    }
  } else if (discountLocked) {
    // Qty and Price can vary.
    const qtySteps = config.quantityMax.sub(config.quantityMin).div(config.quantityStep).floor().toNumber() + 1;
    const priceSteps = priceMax.sub(priceMin).div(config.priceStep).floor().toNumber() + 1;

    if (qtySteps <= priceSteps) {
      for (let q = config.quantityMin; q.lte(config.quantityMax); q = q.add(config.quantityStep)) {
        if (shouldStop()) break;
        processQtyDiscount(q, discountMin);
        count++;
        if (onProgress && count % reportInterval === 0) onProgress(Math.min(count / totalEstimate, 0.99));
      }
    } else {
      for (let p = priceMin; p.lte(priceMax); p = p.add(config.priceStep)) {
        if (shouldStop()) break;
        processPriceDiscount(p, discountMin);
        count++;
        if (onProgress && count % reportInterval === 0) onProgress(Math.min(count / totalEstimate, 0.99));
      }
    }
  } else if (qtyLocked) {
    // Price and Discount can vary.
    const priceSteps = priceMax.sub(priceMin).div(config.priceStep).floor().toNumber() + 1;
    const discountSteps = discountMax.sub(discountMin).div(config.discountStep).floor().toNumber() + 1;

    if (priceSteps <= discountSteps) {
      for (let p = priceMin; p.lte(priceMax); p = p.add(config.priceStep)) {
        if (shouldStop()) break;
        processQtyUnitPrice(config.quantityMin, p);
        count++;
        if (onProgress && count % reportInterval === 0) onProgress(Math.min(count / totalEstimate, 0.99));
      }
    } else {
    ...
    // Large search space, use priority iterator (Yields Qty, Discount)
    ...
    // Standard nested loop for smaller ranges
    for (let q = config.quantityMin; q.lte(config.quantityMax); q = q.add(config.quantityStep)) {
      if (shouldStop()) break;
      for (let d = discountMin; d.lte(discountMax); d = d.add(config.discountStep)) {
        if (shouldStop()) break;
        processQtyDiscount(q, d);
        count++;
        if (onProgress && count % reportInterval === 0) {
          onProgress(Math.min(count / totalEstimate, 0.99));
        }
