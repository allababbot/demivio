...
function calculateHumanScore(result: {
	...
	const priceRef = Math.max(
		1,
		result.transaction.unitPrice.sub(meta.unitPriceDifference).toNumber(),
	);
	...
	const qtyRef = Math.max(
		1,
		result.transaction.quantity.sub(meta.quantityDifference).toNumber(),
	);
	...
	const discRef = Math.max(
		1,
		result.transaction.discount.sub(meta.discountDifference).toNumber(),
	);
	...
	// --- Weighted composite ---
	const composite =
		ppnScore * 0.6 + priceScore * 0.15 + qtyScore * 0.1 + discScore * 0.15;
...
function createResult(
	...
	const metadata: ResultMetadata = {
		...
		dppNilaiLain: transaction.unitPrice
			.mul(transaction.quantity)
			.sub(transaction.discount)
			.mul(11)
			.div(12),
	};
...
function* generatePriorityOrder(
	...
	if (qtyValues.length > 1) {
		...
		qtyValues.sort((a, b) =>
			a.sub(qtyCenter).abs().cmp(b.sub(qtyCenter).abs()),
		);
	}

	if (discountValues.length > 1) {
		const discountCenter = Decimal.max(
			discountMin,
			Decimal.min(discountMax, refDiscount),
		);
		discountValues.sort((a, b) =>
			a.sub(discountCenter).abs().cmp(b.sub(discountCenter).abs()),
		);
	}
...
export function estimateCombinations(config: SimulationConfig): number {
	...
	const quantityCount = qtyLocked
		? 1
		: config.quantityMax
				.sub(config.quantityMin)
				.div(config.quantityStep)
				.floor()
				.toNumber() + 1;
	const discountCount = discountLocked
		? 1
		: config.discountMax
				.sub(config.discountMin)
				.div(config.discountStep)
				.floor()
				.toNumber() + 1;
...
export function estimateCombinationsSimple(config: {
	...
	const quantityCount = Math.max(
		1,
		Math.floor(
			(config.quantityMax - config.quantityMin) / config.quantityStep,
		) + 1,
	);
	const discountCount = Math.max(
		1,
		Math.floor(
			(config.discountMax - config.discountMin) / config.discountStep,
		) + 1,
	);
...
export function runSimulation(
	...
	const processQtyUnitPrice = (quantity: Decimal, unitPrice: Decimal): void => {
		const exactDiscount = calculateRequiredDiscount(
			config.targetPpn,
			unitPrice,
			quantity,
		);
	...
	const processPriceDiscount = (
		unitPrice: Decimal,
		discount: Decimal,
	): void => {
		const exactQty = calculateRequiredQuantity(
			config.targetPpn,
			unitPrice,
			discount,
		);
		...
		const qtysToTry = [
			roundedQty,
			roundedQty.add(config.quantityStep),
			roundedQty.sub(config.quantityStep),
		];

		for (const quantity of qtysToTry) {
			if (
				quantity.lt(config.quantityMin) ||
				quantity.gt(config.quantityMax) ||
				quantity.lte(0)
			) {
				continue;
			}
	...
	const processQtyDiscount = (quantity: Decimal, discount: Decimal): void => {
		...
		for (const unitPrice of pricesToTry) {
			if (
				unitPrice.lt(priceMin) ||
				unitPrice.gt(priceMax) ||
				unitPrice.lte(0)
			) {
				continue;
			}
	...
	if (priceLocked && discountLocked) {
		...
		const qtySteps =
			config.quantityMax
				...
				.toNumber() + 1;
		const discountSteps =
			discountMax.sub(discountMin).div(config.discountStep).floor().toNumber() +
			1;

		if (qtySteps <= discountSteps) {
			for (
				let q = config.quantityMin;
				q.lte(config.quantityMax);
				q = q.add(config.quantityStep)
			) {
				if (shouldStop()) break;
		...
		const qtySteps =
			config.quantityMax
				...
				.toNumber() + 1;
		const priceSteps =
			priceMax.sub(priceMin).div(config.priceStep).floor().toNumber() + 1;
