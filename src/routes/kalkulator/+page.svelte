<script lang="ts">
    ...
    import type { SerializableSimulationConfig, SerializableSimulationResult, WorkerResponse } from "$lib/worker-types";
    ...
    function handleKeydown(e: KeyboardEvent) {
        if (e.ctrlKey && e.key === "Enter") {
            ...
            e.preventDefault();
            if (simResults.length > 0) exportResultsToExcel(simResults, "demivio-kalkulator.xlsx");
        }
    }
    ...
    async function handleSimulate() {
        ...
        const effDiscountMin = isDiscountLocked ? (refDiscount ?? 0) : (discountMin ?? 0);
        const effDiscountMax = isDiscountLocked ? (refDiscount ?? 0) : (discountMax ?? 1000000);
        ...
        const autoDiscountStep = Math.max(
            1,
            Math.ceil(discountRange / Math.max(1, Math.floor(MAX_COMBINATIONS / qtyCount))),
        );
        const autoPriceStep = Math.max(1, Math.ceil(priceRange / Math.max(1, Math.floor(MAX_COMBINATIONS / qtyCount))));
        ...
        // Create Decimal-based config for estimate
        const decimalConfig = {
            priceMin: new Decimal(baseConfig.priceMin),
            priceMax: new Decimal(baseConfig.priceMax),
            discountMin: new Decimal(baseConfig.discountMin),
            discountMax: new Decimal(baseConfig.discountMax),
            discountStep: new Decimal(baseConfig.discountStep),
            quantityMin: new Decimal(baseConfig.quantityMin),
            quantityMax: new Decimal(baseConfig.quantityMax),
            quantityStep: new Decimal(baseConfig.quantityStep),
        };
        ...
        workers.forEach((worker, i) => {
            ...
            // Increment pending workers count for this worker
            pendingWorkers++;
            ...
            worker.onerror = (error) => {
                ...
                simRunning = false;
                // Decrement pending workers on error
                if (pendingWorkers > 0) pendingWorkers--;
                // Decrement active workers on error
                if (activeWorkers > 0) activeWorkers--;
                // Still try to finalize if all workers have completed
                if (activeWorkers === 0 && pendingWorkers === 0) {
                    finalizeSimulation(workerResults, startTime, baseConfig, genId);
                }
            };
            ...
            worker.onmessage = (event: MessageEvent<WorkerResponse>) => {
                ...
                if (response.type === "progress") {
                    ...
                    // Only update progress if we have at least one progress report
                    if (progressMap.size > 0) {
                        const totalProgress =
                            Array.from(progressMap.values()).reduce((a, b) => a + b, 0) / workers.length;
                    ...
                    // Update workerResults for this worker
                    workerResults[i] = response.results;
                    // Decrement pending workers count
                    if (pendingWorkers > 0) pendingWorkers--;
    ...
    function handleCancel() {
        // Increment generation ID to mark this as a new simulation
        currentGenerationId++;
        // Cancel all workers
        workers.forEach((w) => {
        ...
        <aside class="sidebar">
            <div class="input-stack">
                ...
                <div class="action-row">
                    <button class="btn btn-primary btn-run" on:click={handleSimulate} disabled={simRunning}>
                        {#if simRunning}
